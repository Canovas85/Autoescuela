...
export default function EvaluacionExamenTeorico() {
  ...
  const loadData = async () => {
    try {
      const data = await evaluacionExamenesService.getTeorico();
      setRows(data || []);
    } catch (error) {
      setNotification({
        open: true,
        message:
          ...
          "No se pudo cargar la evaluación de examen teórico",
        severity: "error",
      });
    }
  };
      ...
      >
        ...
        <DialogContent sx={{ display: "grid", gap: 1.25, pt: 1 }}>
          ...
          <Typography>
            ...
            {selectedRow?.aciertosExamen ?? "No disponible"}
          </Typography>
          <Typography>
            <strong>Errores:</strong>{" "}
            {selectedRow?.erroresExamen ?? "No disponible"}
          </Typography>
          <Typography>

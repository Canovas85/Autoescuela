  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        flex: 1.8,
        renderCell: (params) => (
          <Stack direction="column" spacing={0.8} sx={{ py: 0.75 }}>
            {(params.row.archivos || []).slice(0, 3).map((archivo) => {
              const fileUrl = buildFileUrl(archivo.ruta);
              const esImagen = isImageFile(
            ...
            {(params.row.archivos || []).length > 3 && (
              <Typography variant="caption" color="text.secondary">
                +{(params.row.archivos || []).length - 3} más
              </Typography>
            )}
          </Stack>
        ),
